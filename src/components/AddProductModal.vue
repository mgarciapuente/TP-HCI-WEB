  ...
  <v-dialog v-model="dialog" max-width="500" persistent>
    <v-card>
      <v-card-title class="d-flex align-center pa-4">
        <v-icon start color="orange">mdi-package-variant-plus</v-icon>
...
const loadCategories = async () => {
  try {
    ...
    const response = await categoriesService.getCategories({ per_page: 50 })
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const newProduct = await productsService.createProduct(createRequest)
...
<style scoped>
:deep(.v-btn--variant-elevated.text-orange) {
  background-color: var(--color-orange) !important;
  color: white !important;
}

:deep(.v-btn--variant-elevated.text-orange:hover) {
